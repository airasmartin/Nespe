...
namespace Nespe.Helpers
{
    ...
    NESTLE,CN=Microsoft Exchange,CN=Services,CN=Configuration,DC=nestle,DC=com
        ...
        public static ActiveDirectoryUserInfo GertUserInfo(string userName)
        {
            var rc = FindUser(userName);
            if (rc.Count < 1)
                return null;
            var o = rc[0];
            var r = new ActiveDirectoryUserInfo
            {
                FirstName = o.Properties["givenname"][0].ToString(),
                Name = o.Properties["name"][0].ToString(),
                EMail = o.Properties["mail"][0].ToString(),
            };
            return r;
        }
    }
